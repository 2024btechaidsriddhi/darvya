...
export default function AlertManagement({ onAlertChange }: AlertManagementProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* List section */}
      <GlassCard glowColor="slate" className="lg:col-span-2 flex flex-col justify-between" id="alert-list-card">
        <div>
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-4">
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                List of active transactions flagged for compliance review.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-slate-900 border border-slate-800 text-[11px] font-sans font-semibold rounded px-2.5 py-1 text-slate-400 focus:outline-none focus:border-slate-700 transition cursor-pointer"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs font-sans text-slate-300">
              <thead>
                <tr className="border-b border-slate-900 text-slate-500 font-sans text-[10px] uppercase tracking-wider font-semibold">
              ...
              <tbody className="divide-y divide-slate-900/60 font-medium">
                {isLoading ? (
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-slate-500 font-sans text-[11px]">
                      Loading active alerts queue...
                    </td>
                  </tr>
                ) : filteredAlerts.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-slate-500 font-sans text-[11px]">
                      No flagged alerts match your filter selection.
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-3 text-center">
                          <span className={`${getRiskScoreClass(alert.riskScore)} font-sans font-semibold`}>
                            {alert.riskScore}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-3">
                          <span className={`px-2 py-0.5 rounded border text-[9px] font-sans font-semibold leading-none ${getStatusBadge(alert.status)}`}>
                            {alert.status}
                          </span>
                        </td>
      ...
      <div className="lg:col-span-1">
        {selectedAlert ? (
          <GlassCard glowColor="slate" className="h-full flex flex-col justify-between" id="alert-detail-sidebar">
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-start border-b border-slate-900 pb-3">
                ...
                <span className={`px-2 py-1 text-[9px] rounded border font-sans font-semibold uppercase ${getStatusBadge(selectedAlert.status)}`}>
              ...
              <div className="p-3 bg-slate-900/40 border border-slate-900 rounded-lg space-y-1 text-xs">
                <div>
                  <span className="text-slate-500 text-[10px] font-sans uppercase block">Target Account</span>
                  <span className="font-bold text-slate-200 block">{selectedAlert.accountName}</span>
                </div>
                <div className="flex justify-between text-[11px] font-sans text-slate-500 mt-1">
                  <span>ID: {selectedAlert.accountId}</span>
                  <span className={getRiskScoreClass(selectedAlert.riskScore)}>Risk Score: {selectedAlert.riskScore}</span>
                </div>
              </div>

              {/* Alert Classification Details */}
              <div>
                <span className="text-slate-500 text-[10px] font-sans uppercase block">Trigger Category</span>
                <span className="font-bold text-xs text-slate-300 font-sans mt-1 block">
              ...
              <div>
                <span className="text-slate-500 text-[10px] font-sans uppercase block">System Summary</span>
                <p className="text-[11px] font-sans text-slate-300 bg-slate-900/60 p-3 border border-slate-900/80 rounded h-[110px] overflow-y-auto leading-relaxed mt-1">
                  {selectedAlert.description}
                </p>
              </div>

              {/* Investigator Designation */}
              <div className="text-[11px] font-sans text-slate-400 flex items-center gap-1.5 bg-slate-900/30 px-3 py-1.5 border border-slate-900 rounded-lg">
            ...
            <div className="border-t border-slate-900 pt-4 mt-6">
              <span className="text-slate-500 text-[10px] font-sans uppercase block mb-2.5">Action Panel</span>
              ...
              <div className="space-y-2">
                ...
                {!selectedAlert.assignee && selectedAlert.status !== 'Closed' && (
                  <button 
                    ...
                    className="w-full py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-[11px] font-sans font-semibold rounded-lg flex items-center justify-center gap-1.5 transition cursor-pointer"
                  >
                    <UserPlus className="w-3.5 h-3.5" /> Assign to Self
                  </button>
                )}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {selectedAlert.status !== 'Escalated' && (
                      <button 
                        ...
                        className="py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-[11px] font-sans font-semibold rounded-lg flex items-center justify-center gap-1.5 transition cursor-pointer"
                      >
                    ...
                    {selectedAlert.status !== 'Frozen' && (
                      <button 
                        ...
                        className="py-2 bg-rose-950/10 hover:bg-rose-950/20 border border-rose-900/20 text-rose-400 text-[11px] font-sans font-semibold rounded-lg flex items-center justify-center gap-1.5 transition cursor-pointer"
                      >
                ...
                {selectedAlert.status === 'Closed' && (
                  <div className="text-center py-2 bg-slate-950 border border-slate-900 rounded-lg text-xs font-sans text-emerald-500 flex items-center justify-center gap-1.5 leading-none">
                    <ShieldCheck className="w-4 h-4" /> Alert Resolved
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="h-full flex items-center justify-center border border-slate-900 bg-slate-950/30 rounded-xl py-24 px-4 text-center">
            <span className="text-xs font-sans text-slate-500">Please select an individual alert row to inspect logs and perform action triggers.</span>
          </div>
        )}
      </div>
    </div>
