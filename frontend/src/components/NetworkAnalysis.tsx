    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      ...
      <div className="lg:col-span-1 space-y-4">
        <GlassCard glowColor="slate" className="h-full flex flex-col justify-between">
          <div className="space-y-4">
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-1">
                Adjust display options for the transaction relationship graph.
              </p>
            </div>
            ...
            <div className="space-y-3.5 border-t border-slate-900 pt-4">
              ...
              <button 
                ...
                className={`w-full p-2.5 rounded-lg border text-xs font-sans font-semibold flex items-center justify-between transition cursor-pointer
                  ${communityView 
              ...
              >
                <span className="flex items-center gap-2">
                  <Grid3X3 className="w-4 h-4" /> Group by Communities
                </span>
              ...
              >
                <span className="flex items-center gap-2">
                  <ShieldAlert className="w-4 h-4" /> Highlight Risk Nodes
                </span>
                <span className={`w-2 h-2 rounded-full ${highlightFraudRings ? 'bg-rose-500 shadow-sm shadow-rose-500' : 'bg-slate-700'}`} />
              </button>
            ...
            {/* Quick legend */}
            <div className="p-3 bg-slate-900/30 border border-slate-900 rounded-lg space-y-2 text-[10px] font-sans">
              <span className="text-slate-500 block uppercase font-bold mb-1">Graph Legend</span>
              <div className="flex items-center gap-2 text-slate-400">
                <span className="w-2.5 h-2.5 rounded-full bg-rose-500 shrink-0" />
                <span>High Risk Seed</span>
              </div>
              <div className="flex items-center gap-2 text-slate-400">
          ...
          <div className="text-[10px] text-slate-500 leading-relaxed font-sans mt-4 flex items-center gap-1.5 border-t border-slate-900 pt-3">
            <HelpCircle className="w-3.5 h-3.5 text-slate-500" /> Pan via dragging the background. Click nodes to focus profiles.
          </div>
        </GlassCard>
      </div>
      ...
      <div className="lg:col-span-3 space-y-6">
        <div className="relative">
          <GlassCard glowColor="slate" className="p-0 overflow-hidden relative border-slate-800 bg-slate-950/80 h-[480px]">
            ...
            <div className="absolute top-4 right-4 z-20 bg-slate-950/80 border border-slate-900 px-3 py-1 font-sans text-[9px] text-slate-500 rounded-md">
            ...
            >
              <defs>
                ...
                <pattern id="cyber-grid" width="30" height="30" patternUnits="userSpaceOnUse">
                  <circle cx="15" cy="15" r="0.75" fill="rgba(148, 163, 184, 0.15)" />
                </pattern>
              ...
              <g transform={`translate(${200 + panX}, ${100 + panY}) scale(${zoom})`}>
                ...
                {suspiciousClusters && (
                  <>
                    {/* Ring 1 - Moscow laundering group */}
                    <motion.circle 
                  ...
                  return (
                    <g key={`edge-${index}`}>
                      <motion.line
                        ...
                        x2={x2}
                        y2={y2}
                        stroke={edge.isSuspicious ? 'rgba(239, 68, 68, 0.4)' : '#334155'}
                      ...
                      <text
                        x={(x1 + x2) / 2}
                        y={(y1 + y2) / 2 - 4}
                        fill={edge.isSuspicious ? '#ef4444' : '#64748b'}
                        fontSize="7"
                        textAnchor="middle"
                        fontFamily="sans-serif"
                        fontWeight="semibold"
                ...
                {/* Draw highlighted risk node indicators (without ping) */}
          ...
          {selectedNode && (
            <motion.div 
              ...
              className="absolute bottom-4 left-4 right-4 z-20 p-4 bg-slate-950/95 border border-slate-800 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 backdrop-blur-md"
              id="selected-node-panel"
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] bg-slate-900 border border-slate-800 px-1.5 py-0.5 rounded font-sans text-slate-300 font-semibold">
                    {selectedNode.id}
                  </span>
                  <span className={`text-[9px] font-sans font-semibold px-1.5 py-0.5 rounded uppercase
                ...
                <span className="text-[10px] text-slate-400 font-sans mt-0.5 block truncate">
                  Community Group: Community #{selectedNode.community} ({selectedNode.type} layer)
                </span>
              </div>
              ...
              <div className="flex gap-2 shrink-0">
                <button 
                  ...
                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded font-sans text-[10px] text-slate-400 cursor-pointer"
                >
                  Unfocus Node
                </button>
              </div>
